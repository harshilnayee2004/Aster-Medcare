...
export default function HeightPassTemplate({ hideActions = false, patient: propPatient }) {
  ...
  useEffect(() => {
    ...
    async function fetchPdf() {
      try {
        ...
        const actualForm = forms["5-form-height-pass"]?.data || {};
        ...
        const response = await api.post(`/forms/fill/5-form-height-pass`, { values }, {
          responseType: "blob"
        });

        const outputFilename = `filled_5_FORM_Height_Pass_${patient.patientId}.pdf`;
        ...
        console.error("Failed to generate Height Pass PDF:", err);
      } finally {
        setLoadingPdf(false);
      }
    }
  ...
  if (loading) {
    return <div className="text-center py-20 text-slate-500 font-semibold">Loading Height Pass Report...</div>;
  }
    ...
    <main className={hideActions ? "" : "template-screen"}>
      {!hideActions && (
        <div className="template-actions print:hidden">
          <Link to={`/patients/${patientId}/height-pass`} className="button-secondary">Back to Form</Link>
      ...
      {loadingPdf ? (
        ...
        <div className="text-center py-20 text-red-500 font-semibold">Failed to load Height Pass PDF template.</div>
      )}
    </main>
  );
}
