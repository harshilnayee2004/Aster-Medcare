...
export default function OphthalForm6Template({ hideActions = false, patient: propPatient }) {
  ...
  useEffect(() => {
    ...
    async function fetchPdf() {
      try {
        ...
        const actualForm = forms["10-form-ophthal-form-6"]?.data || {};

        // Stamping mapping
        const values = {
          dateTop: formatDateToDMY(actualForm.dateTop || ""),
          department: actualForm.department || "",
          ...
          dateOfEmployment: formatDateToDMY(actualForm.dateOfEmployment || ""),
          examinationDate: formatDateToDMY(actualForm.examinationDate || ""),
          examinationResult: actualForm.examinationResult || "",
          remarks: actualForm.remarks || "",
          doctorSignatureRow: actualForm.doctorSignatureRow || "",
          dateBottom: formatDateToDMY(actualForm.dateBottom || ""),
          doctorStamp: actualForm.doctorStamp || "",
        };

        const response = await api.post(`/forms/fill/10-form-ophthal-form-6`, { values }, {
          responseType: "blob"
        });

        const outputFilename = `filled_Ophthalmic_Form_6_${patient.patientId}.pdf`;
        ...
        console.error("Failed to generate Ophthalmic Form 6 PDF:", err);
      } finally {
        setLoadingPdf(false);
      }
    }
  ...
  if (loading) {
    return <div className="text-center py-20 text-slate-500 font-semibold">Loading Ophthalmic Form 6 Report...</div>;
  }
    ...
    <main className={hideActions ? "" : "template-screen"}>
      {!hideActions && (
        <div className="template-actions print:hidden">
          <Link to={`/patients/${patientId}/ophthal-form-6`} className="button-secondary">Back to Form</Link>
      ...
      {loadingPdf ? (
          ...
          {hideActions && (
            <div data-html2canvas-ignore="true" className="flex items-center justify-between border-b border-line pb-3 print:hidden">
              <h3 className="text-sm font-bold text-slate-800">
                Ophthalmic Form 6 PDF Document (Stamped)
              </h3>
              <button
        ...
        <div className="max-w-[800px] mx-auto bg-red-50 p-12 border border-red-200 text-red-700 rounded-xl text-center font-medium print:hidden">
          Failed to load Ophthalmic Form 6 PDF template. Please check configuration.
        </div>
      )}
    </main>
  );
