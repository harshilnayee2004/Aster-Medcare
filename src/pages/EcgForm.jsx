...
export default function EcgForm() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        try {
          const res = await api.get(`/patients/${patientId}/forms/19-form-ecg`);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        await updatePatientForm(patientId, "19-form-ecg", form);
        setSaveStatus("Saved ✓");
      } catch (err) {
  ...
  function handlePhoto(file) {
    ...
    reader.onload = () => {
      setForm({ ecgPhoto: reader.result });
    };
  ...
  function preview() {
    save();
    navigate(`/patients/${patientId}/ecg/preview`);
  }
      ...
      <form onSubmit={submit} className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="sticky top-24 z-10 bg-white/95 backdrop-blur-md p-4 border border-slate-100 rounded-2xl shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <span className="text-xxs font-extrabold uppercase tracking-widest text-slate-400">19 FORM ECG</span>
          ...
          <div className="flex items-center gap-3 self-end sm:self-auto">
            ...
            <button type="button" onClick={preview} className="inline-flex h-10 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700 hover:bg-slate-50 transition shadow-sm" disabled={!form.ecgPhoto}>Preview Report</button>
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm space-y-6">
          <div className="pb-3 border-b border-slate-100">
            <h2 className="text-base font-bold text-slate-800 tracking-tight">ECG Scan Upload</h2>
            <p className="text-xxs text-slate-400 mt-0.5">Please upload an image file of the worker's ECG report.</p>
          </div>
          
          <div>
            <label className="field-label font-bold text-slate-600 mb-2">ECG Image File</label>
            {!form.ecgPhoto ? (
              <div className="flex justify-center rounded-2xl border border-dashed border-slate-300 px-6 py-12 transition hover:border-brand bg-slate-50/50">
                <div className="text-center">
                  ...
                  <div className="mt-4 flex text-sm text-slate-600 justify-center">
                    <label htmlFor="ecg-file-upload" className="relative cursor-pointer rounded-md font-bold text-brand focus-within:outline-none hover:text-blue-700">
                      <span>Upload a file</span>
                      <input id="ecg-file-upload" name="ecg-file-upload" type="file" accept="image/*" className="sr-only" onChange={(e) => handlePhoto(e.target.files[0])} />
                    </label>
              ...
              <div className="relative rounded-2xl border border-slate-100 bg-slate-50/50 p-4">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xs font-bold text-slate-500">Uploaded ECG Preview:</span>
                  <button type="button" onClick={removePhoto} className="text-xs font-bold text-red-600 hover:text-red-800 transition">
                    Remove Image
                  </button>
                </div>
                <div className="flex justify-center bg-slate-900 rounded-xl p-4 max-h-[600px] overflow-hidden">
                  <img src={form.ecgPhoto} alt="ECG Preview" className="max-h-[560px] max-w-full object-contain shadow-md rounded" />
                </div>
              </div>
            )}
          </div>
