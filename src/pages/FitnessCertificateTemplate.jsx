...
export default function FitnessCertificateTemplate({ hideActions = false, patient: propPatient }) {
  ...
  useEffect(() => {
    ...
    async function fetchPdf() {
      try {
        ...
        const actualForm = forms["25-form-for-medical-fitness-certificate-format"]?.data || {};

        // Stamping mapping
        const values = {
          name: actualForm.name || "",
          parentName: actualForm.parentName || "",
          age: actualForm.age || "",
          residentOfLine1: actualForm.residentOfLine1 || "",
          residentOfLine2: actualForm.residentOfLine2 || "",
          residentOfLine3: actualForm.residentOfLine3 || "",
          marksOfIdentification: actualForm.marksOfIdentification || "",
          signatureCandidate: actualForm.signatureCandidate || "",
          leftThumb: actualForm.leftThumb || "",
          signatureMedicalOfficer: actualForm.signatureMedicalOfficer || "",
          nameMedicalOfficer: actualForm.nameMedicalOfficer || "",
          registrationNo: actualForm.registrationNo || "",
          date: formatDateToDMY(actualForm.date || ""),
          seal: actualForm.seal || "",
        };

        const response = await api.post(`/forms/fill/25-form-for-medical-fitness-certificate-format`, { values }, {
          responseType: "blob"
        });

        const outputFilename = `filled_Fitness_Certificate_${patient.patientId}.pdf`;
        ...
        console.error("Failed to generate Fitness Certificate PDF:", err);
      } finally {
        setLoadingPdf(false);
      }
    }
  ...
  if (loading) {
    return <div className="text-center py-20 text-slate-500 font-semibold">Loading Medical Fitness Certificate...</div>;
  }
    ...
    <main className={hideActions ? "" : "template-screen"}>
      {!hideActions && (
        <div className="template-actions print:hidden">
          <Link to={`/patients/${patientId}/fitness-certificate`} className="button-secondary">Back to Form</Link>
      ...
      {loadingPdf ? (
          ...
          {hideActions && (
            <div data-html2canvas-ignore="true" className="flex items-center justify-between border-b border-line pb-3 print:hidden">
              <h3 className="text-sm font-bold text-slate-800">
                Medical Fitness Certificate PDF Document (Stamped)
              </h3>
              <button
        ...
        <div className="max-w-[800px] mx-auto bg-red-50 p-12 border border-red-200 text-red-700 rounded-xl text-center font-medium print:hidden">
          Failed to load Medical Fitness Certificate PDF template. Please check configuration.
        </div>
      )}
    </main>
  );
