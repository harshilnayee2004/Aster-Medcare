...
const ALL_24_FORMS = [
  ...
  { key: "postMedical", label: "22 FORM Post Medical" },
  { key: "eyeExam", label: "Ophthalmological Examination Report (Front)" },
  { key: "form33", label: "Form No. 33 (Fitness Certificate)" },
  { key: "healthRegister", label: "Form No. 32 (Health Register)" },
  { key: "xrayReport", label: "X-Ray Report" },
  { key: "4-form-airport-bohw", label: "4 FORM Airport BOHW" },
...
export default function AdminPanel() {
  ...
  const handleSelectAll = (category) => {
    ...
    const keysToSelect = category === "built" 
      ? ALL_24_FORMS.slice(0, 19).map(f => f.key)
      : ALL_24_FORMS.slice(19).map(f => f.key);
  ...
  const builtForms = ALL_24_FORMS.slice(0, 19);
  const placeholderForms = ALL_24_FORMS.slice(19);
