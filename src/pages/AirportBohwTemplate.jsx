...
export default function AirportBohwTemplate({ hideActions = false, patient: propPatient }) {
  ...
  useEffect(() => {
    ...
    async function fetchPdf() {
      try {
        ...
        const actualForm = forms["4-form-airport-bohw"]?.data || {};
        ...
        const response = await api.post(`/forms/fill/4-form-airport-bohw`, { values }, {
          responseType: "blob"
        });

        const outputFilename = `filled_4_FORM_Airport_BOHW_${patient.patientId}.pdf`;
        ...
        console.error("Failed to generate Form XI PDF:", err);
      } finally {
        setLoadingPdf(false);
      }
    }
  ...
  if (loading) {
    return <div className="text-center py-20 text-slate-500 font-semibold">Loading Form XI Report...</div>;
  }
    ...
    <main className={hideActions ? "" : "template-screen"}>
      {!hideActions && (
        <div className="template-actions print:hidden">
          <Link to={`/patients/${patientId}/airport-bohw`} className="button-secondary">Back to Form</Link>
      ...
      {loadingPdf ? (
        ...
        <div className="text-center py-20 text-red-500 font-semibold">Failed to load Form XI PDF template.</div>
      )}
    </main>
  );
}
