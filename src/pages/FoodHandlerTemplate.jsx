...
export default function FoodHandlerTemplate({ hideActions = false, patient: propPatient }) {
  ...
  useEffect(() => {
    ...
    async function fetchPdf() {
      try {
        ...
        const actualForm = forms["17-form-food-handler-certificate"]?.data || {};
        ...
        const values = {
          ...
          specificNotes: actualForm.specificNotes || "",
          patientSignature: patient.signature || "",
          doctorName: actualForm.doctorName || ""
        };

        const response = await api.post(`/forms/fill/17-form-food-handler-certificate`, { values }, {
          responseType: "blob"
        });

        const outputFilename = `filled_Food_Handler_Certificate_${patient.patientId}.pdf`;
        ...
        console.error("Failed to generate Food Handler Certificate PDF:", err);
      } finally {
        setLoadingPdf(false);
      }
    }
    ...
    <main className={hideActions ? "" : "template-screen"}>
      {!hideActions && (
        <div className="template-actions print:hidden">
          <Link to={`/patients/${patientId}/food-handler`} className="button-secondary">Back to Form</Link>
          ...
          {hideActions && (
            <div data-html2canvas-ignore="true" className="flex items-center justify-between border-b border-line pb-3 print:hidden">
              <h3 className="text-sm font-bold text-slate-800">
                Food Handler Certificate PDF Document (Stamped)
              </h3>
              <button
