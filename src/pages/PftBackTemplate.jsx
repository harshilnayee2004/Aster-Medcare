...
export default function PftBackTemplate({ hideActions = false, patient: propPatient }) {
  ...
  useEffect(() => {
    ...
    async function fetchPdf() {
      try {
        ...
        const actualForm = forms["14-form-pft-back"]?.data || {};
        ...
        const values = {
          ...
          peakFlowReading: actualForm.peakFlowReading || "",
          zoneIndication: actualForm.zoneIndication || "",
          recommendation: actualForm.recommendation || "No Any.",
          pftChart: actualForm.pftChart || "",
          conclusionComment: actualForm.conclusionComment || "Normal Peak Flow and Expiratory flow rate test.",
          date: formatDateToDMY(actualForm.date || ""),
        };

        const response = await api.post(`/forms/fill/14-form-pft-back`, { values }, {
          responseType: "blob"
        });

        const outputFilename = `filled_PFT_Back_${patient.patientId}.pdf`;
        ...
        console.error("Failed to generate PFT Back PDF:", err);
      } finally {
        setLoadingPdf(false);
      }
    }
      ...
      {!hideActions && (
        <div className="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-sm print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-slate-800">14 FORM PFT Back - Preview</h1>
