...
export default function HeightPassForm() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        try {
          const res = await api.get(`/patients/${patientId}/forms/5-form-height-pass`);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        await updatePatientForm(patientId, "5-form-height-pass", form);
        setSaveStatus("Saved ✓");
      } catch (err) {
  ...
  function preview() {
    save();
    navigate(`/patients/${patientId}/height-pass/preview`);
  }
        ...
        <div className="sticky top-24 z-10 bg-white/95 backdrop-blur-md p-4 border border-slate-100 rounded-2xl shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <span className="text-xxs font-extrabold uppercase tracking-widest text-slate-400">Height Pass Test Report</span>
          ...
          <Section title="1. Candidate Demographics & Vitals">
            ...
            <div className="col-span-1 md:col-span-2 lg:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-5 border-t border-slate-100 pt-4 mt-2">
              <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 space-y-3">
                <span className="field-label font-bold text-slate-700 block border-b border-slate-200/50 pb-1.5">Before Height Pass</span>
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 space-y-3">
                <span className="field-label font-bold text-slate-700 block border-b border-slate-200/50 pb-1.5">After Height Pass</span>
                <div className="grid grid-cols-2 gap-4">
