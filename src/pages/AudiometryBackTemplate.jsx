...
export default function AudiometryBackTemplate({ hideActions = false, patient: propPatient }) {
  ...
  useEffect(() => {
    ...
    async function fetchPdf() {
      try {
        ...
        const actualForm = forms["12-form-audiometry-back"]?.data || {};
        ...
        const values = {
          ...
          comments: actualForm.comments || "No Any.",
          diagnosis: actualForm.diagnosis || "Normal Hearing Test in both ear.",
          recommendation: actualForm.recommendation || "No Any.",
          date: formatDateToDMY(actualForm.date || ""),
          audiometryChart: actualForm.audiometryChart || "",
        };

        const response = await api.post(`/forms/fill/12-form-audiometry-back`, { values }, {
          responseType: "blob"
        });

        const outputFilename = `filled_Audiometry_Back_${patient.patientId}.pdf`;
        ...
        console.error("Failed to generate Audiometry Back PDF:", err);
      } finally {
        setLoadingPdf(false);
      }
    }
      ...
      {!hideActions && (
        <div className="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-sm print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-slate-800">12 FORM Audiometry Back - Preview</h1>
