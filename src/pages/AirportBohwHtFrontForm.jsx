...
const initialAirportBohwHtFront = {
  certificateSerialNo: "",
  dateTop: "",
  name: "",
  identificationMarks: "",
...
export default function AirportBohwHtFrontForm() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        try {
          const res = await api.get(`/patients/${patientId}/forms/35-form-airport-bohw-ht-front`);
          if (res.data && res.data.formExists) {
            setForm({
              ...initialAirportBohwHtFront,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        await updatePatientForm(patientId, "35-form-airport-bohw-ht-front", form);
        setSaveStatus("Saved ✓");
      } catch (err) {
  ...
  function preview() {
    save();
    navigate(`/patients/${patientId}/airport-bohw-ht-front/preview`);
  }
        ...
        <div className="sticky top-24 z-10 bg-white/95 backdrop-blur-md p-4 border border-slate-100 rounded-2xl shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <span className="text-xxs font-extrabold uppercase tracking-widest text-slate-400">Airport BOHW-HT Front</span>
